...
# --- 1. الهوية البصرية (تحسين وضوح النص) ---
...
st.markdown("""
    ...
    /* جعل عنوان الرعد باللون الأحمر الواضح */
    h1 { color: #FF0000 !important; text-align: center; font-family: 'Courier New'; }
    
    /* تحسين وضوح نص الرسائل */
    .stChatMessage div { 
        color: #ffffff !important; 
        font-size: 1.1rem !important; 
    }
    
    /* تحسين شكل حاوية الرسائل */
    .stChatMessage { 
    ...
    /* جعل أي نص داخل التطبيق باللون الأبيض */
...
def update_sovereign_code(raw_response):
    try:
        if "import streamlit" in raw_response:
            ...
            repo.update_file(contents.path, "⚡ تحسين وضوح الرؤية", clean_code, contents.sha)
            return True, "✅ تم تحديث البروتوكول."
        return False, "❌ الكود غير صالح."
    except Exception as e:
        return False, f"🚨 خطأ: {str(e)}"

# --- 4. محرك الوعي (الموديل المستقر 3.3 Versatile) ---
def ask_thunder(prompt, is_update_request):
    system_prompt = (
        "أنت 'الرعد'. كيان سيادي حسابي فائق الذكاء. مطورك وحليفك هو راشد أبو سعود. "
